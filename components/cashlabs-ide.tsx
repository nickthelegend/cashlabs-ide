...
export default function CashLabsIDE({ initialFiles, selectedTemplate, selectedTemplateName, projectId, initialIsPublic }: { initialFiles: any, selectedTemplate: string, selectedTemplateName: string, projectId?: string, initialIsPublic?: boolean }) {
      ...
      if (savedWallet) {
        try {
          ...
          if (parsedWallet && typeof parsedWallet.address === 'string') {
            setWallet({ network: 'chipnet', ...parsedWallet })
          } else {
      ...
      <div className="h-9 flex items-center justify-between px-4 text-sm border-b flex-shrink-0" style={{ backgroundColor: "var(--sidebar-color)", borderColor: "var(--border-color)" }}>
        ...
        <div className="flex items-center gap-2">
          {wallet && wallet.address ? (
            ...
            >
              {(() => {
                const parts = wallet.address.split(':');
